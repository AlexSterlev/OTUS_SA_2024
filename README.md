...
<a name="testc"><h3>Тест кейсы для задач MVP</h3></a>

<details>

<summary><h3>Тестовый сценарий: Ввод учетных данных на странице входа в систему.</h3></summary>
	
<h4>Цель: Убедиться, что пользователь может успешно ввести свои учетные данные (логин и пароль) на странице входа в систему.</h4>
...
Тестовый сценарий: Проверка корректности учетных данных при авторизации

<h4>Цель: Убедиться, что система проверяет введенные учетные данные и авторизует пользователя, только если они корректны.</h4>
...
Тестовый сценарий: Проверка доступа к интерфейсу управления контентом после успешной авторизации

<h4>Цель: Убедиться, что после успешной авторизации пользователь имеет доступ к интерфейсу управления и отслеживания проигрываемого контента.</h4>
...
<h4>Цель: Убедиться, что система обеспечивает безопасность данных и защиту от несанкционированного доступа к учетным данным других пользователей.</h4>
...
Тестовый сценарий: Выход из системы (разлогинивание)

<h4>Цель: Убедиться, что пользователь может безопасно завершить текущую сессию работы с контентом.</h4>
...
<h4>Цель: Убедиться, что система предоставляет механизм для отслеживания активности пользователя.</h4>
...
Тестовый сценарий: Обеспечение безопасного завершения сессии в случае технических проблем

<h4>Цель: Убедиться, что система обеспечивает безопасное завершение текущей сессии пользователя в случае возникновения технических проблем или сбоев.</h4>
...
<h4>Цель: Убедиться, что пользователь может запросить восстановление доступа к своему аккаунту в случае утери или забытого пароля через механизм восстановления пароля по электронной почте.</h4>
...
<summary><h3>Тестовый сценарий: Загрузка плейлистов и видеофайлов через интерфейс управления контентом</h3></summary>
	
<h4>Цель: Убедиться, что пользователь может получить доступ к интерфейсу управления контентом, где есть опция загрузки плейлистов и видео.</h4>
...
Тестовый сценарий: Выбор файлов с локального компьютера или облачного хранилища

<h4>Цель: Убедиться, что пользователь может выбрать плейлист или видео файлы с локального компьютера или из облачного хранилища.</h4>
...
Тестовый сценарий: Поддержка различных форматов файлов

<h4>Цель: Убедиться, что система поддерживает различные форматы видео файлов и плейлистов.</h4>
...
Тестовый сценарий: Указание терминалов для загрузки контента

<h4>Цель: Убедиться, что после выбора файлов пользователь может указать терминалы, на которые нужно загрузить контент.</h4>
...
Тестовый сценарий: Выбор нескольких терминалов для загрузки контента

<h4>Цель: Убедиться, что пользователь может выбрать несколько терминалов для одновременной загрузки контента.</h4>
...
Тестовый сценарий: Уведомление о статусе загрузки и прогрессе

<h4>Цель: Убедиться, что пользователь получает уведомления о статусе загрузки каждого файла на каждый выбранный терминал, а также о прогрессе загрузки.</h4>
...
Тестовый сценарий: Обработка ошибок и проблем в процессе загрузки
...
Тестовый сценарий: Просмотр, редактирование и удаление загруженных файлов
...
Тестовый сценарий: Безопасное хранение файлов и контроль доступа
